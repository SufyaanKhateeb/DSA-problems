...
public:
    ...
    int value_n_from_end(int n) {
        // return value_at(size - n); // can just do this if there is size and value_at function.
        if (head == NULL)
            return -3000;
        if (n == 0) {
            cout << "Invalid index.";
...
int main() {
    ...
    // Delete operations
        ...
        // erase(index) - removes node at given index
        // remove_value(value) - removes the first item in the list with this value
        // remove_nth_from_end(index) - removes the nth element form the end (1 based index)

    mylist.pop_front();
    mylist.pop_back();
    mylist.erase(2);
    mylist.remove_value(40);
    mylist.remove_nth_from_end(2);
    mylist.printLL();
    ...
    cout << "Element at nth position from end: " << mylist.value_n_from_end(1) << endl;
    mylist.printLL();
